...
export default function Portfolio() {
  ...
  // ── Tech stack with devicon class names ──────────────────
  const techStacks = [
    ...
    { name:"Microsoft Power Bi",      img:"/Power Bi.svg",             },
    { name:"KNIME",      img:"/knime.svg",             },
    { name:"Ultralytics-YOLO",      img:"/Ultralytics-YOLO.svg",             },
    { name:"Hugging Face",      img:"/hf-logo.svg",             },
    { name:"Dobot",      img:"/Dobot.svg",             },
  ...
  const timeline = [
    {
      year: "2022",
      type: "edu",
      title: "เข้าศึกษา ม.บูรพา",
      sub: "B.Sc. Applied AI & Smart Technology",
      detail: "เรียนรู้ Machine Learning, Predictive Analytics, Robotics & Automation, Software Architecture",
    },
    {
      year: "2024",
      type: "work",
      title: "Software & AI Intern",
      sub: "Sinfonia Technology (Thailand) Co., Ltd.",
      detail: "8 เดือน — Torque Data System (VB.NET/RS232), Wire Detection (YOLO), Stock Forecasting (Linear Regression)",
    },
    {
      year: "2025",
      type: "open",
      title: "Open to opportunities",
      sub: "AI · Data · Software Engineering",
      detail: "พร้อมร่วมงานทันที — ทั้งในไทยและ remote",
    },
  ];

  const typeColor: Record<string,string> = {
    edu: "#5b8dd9", mentor: "#c8a96e", work: "#5dba8a", open: "#a87fc8",
  };
  ...
  const card = (highlight=false) => ({ border:`0.5px solid ${highlight ? t.highlightBd : t.borderCard}`, padding:"1.75rem", background:highlight ? t.highlightBg : t.bgCard, transition:"border-color 0.2s, background 0.3s" });
  ...
  return (
    <main style={{ fontFamily:serif, background:t.bg, color:t.text, minHeight:"100vh", opacity:visible?1:0, transition:"opacity 0.5s ease, background 0.3s ease, color 0.3s ease" }}>
      ...
      <nav style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"1.1rem 2.5rem", borderBottom:`0.5px solid ${t.border}`, position:"sticky", top:0, background:t.bgNav, backdropFilter:"blur(10px)", zIndex:50, transition:"background 0.3s" }}>
        <span style={{ fontFamily:mono, fontSize:"12px", letterSpacing:"0.14em", color:t.accent }}>
          SP<span style={{ color:t.textFaintest }}>_</span>PORTFOLIO
        </span>
      ...
      <section id="about" style={{ padding:"5rem 2.5rem 4rem", borderBottom:`0.5px solid ${t.border}`, maxWidth:"1100px", margin:"0 auto" }}>
        <div style={{ display:"flex", flexWrap:"wrap", gap:"3rem", alignItems:"center", justifyContent:"space-between" }}>
          ...
          <div className="fu d3" style={{ flex:"0 0 240px", display:"flex", justifyContent:"center" }}>
            <div style={{ width:"240px", height:"240px", borderRadius:"50%", padding:"8px", border:`1px dashed ${t.borderCard}`, background:t.bgCard, display:"flex", alignItems:"center", justifyContent:"center" }}>
               {/* แก้ path src ให้ตรงกับไฟล์รูปภาพของคุณ (ควรไว้ในโฟลเดอร์ public) */}
               <img 
                 src="/profile.jpg" 
                 alt="Suwat Punkhiew" 
        ...
        <div style={{ display:"flex", gap:"0", marginTop:"4rem", borderTop:`0.5px solid ${t.border}`, paddingTop:"2rem", flexWrap:"wrap" }}>
          {[
            ...
            { num:"2022-2026", label:"B.Sc. Applied AI, Burapha U." },
          ].map((s,i,arr) => (
      ...
      <section id="projects" style={{ padding:"4rem 2.5rem", borderBottom:`0.5px solid ${t.border}`, maxWidth:"1100px", margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
          {projects.map(p => (
            <div key={p.id} style={card(p.highlight)}
      ...
      <section id="skills" style={{ padding:"4rem 2.5rem", borderBottom:`0.5px solid ${t.border}`, maxWidth:"1100px", margin:"0 auto" }}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:"10px", marginBottom:"3rem" }}>
          {techStacks.map(tech => (
            <div key={tech.name}
              ...
              {/* --- เปลี่ยนโค้ดตรงนี้ --- */}
              {tech.icon ? (
                // ถ้ามีคีย์ icon ให้ใช้แบบ Font (Devicon)
                <i className={tech.icon} style={{ fontSize:"28px", color:tech.color }} />
              ) : (
                // ถ้าไม่มี ให้ดึงรูปภาพจาก path ในคีย์ img มาแสดงแทน
                <img src={tech.img} alt={tech.name} style={{ width:"28px", height:"28px", objectFit:"contain" }} />
              )}
              {/* ---------------------- */}
        ...
        {/* Education */}
        <div style={{ marginTop:"3rem", padding:"1.5rem", border:`0.5px solid ${t.borderCard}`, background:t.bgCard, transition:"background 0.3s" }}>
      ...
      <section style={{ padding:"4rem 2.5rem", borderBottom:`0.5px solid ${t.border}`, maxWidth:"1100px", margin:"0 auto" }}>
        ...
        <div style={{ border:`0.5px solid ${t.borderCard}`, padding:"1.25rem", background:t.bgCard, display:"flex", flexDirection:"column", gap:"10px" }}>
          ...
          <img
            src={`https://ghchart.rshah.org/c8a96e/NothingToSad`}
